...
folder are mapped to the application's root namespace, 'appRudentsov'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "appRudentsov"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'appRudentsov' in the same way
as `"appRudentsov/sass/src"`.
